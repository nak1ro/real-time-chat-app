...
export function useToggleReaction(options?: {
  ...
  return useMutation({
    mutationFn: async ({ messageId, emoji }: { messageId: string; emoji: string }): Promise<ToggleReactionResponse> => {
      const socket = getConnectedSocket();
...
export function useMarkAsRead(options?: {
  ...
  return useMutation({
    mutationFn: async ({ conversationId, upToMessageId }: { conversationId: string; upToMessageId: string }): Promise<BulkReceiptUpdate> => {
      const socket = getConnectedSocket();
