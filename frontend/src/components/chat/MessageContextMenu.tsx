...
export function MessageContextMenu({
  ...
  const menuItemCount = 2 + (canEdit ? 1 : 0) + (canDelete ? 1 : 0); // Reply + Copy + optional Edit + optional Delete
  ...
  const calculatePosition = useCallback(() => {
    ...
    // Own messages: menu appears on left side of click point
    // Other's messages: menu appears on right side of click point
    ...
    if (horizontalPlacement === 'left') {
      ...
      // Try to place menu to the right of click point
      if (spaceRight >= MENU_WIDTH + VIEWPORT_PADDING) {
        x = position.x;
      } else if (spaceLeft >= MENU_WIDTH + VIEWPORT_PADDING) {
        // Fall back to left side if not enough space on right
        x = position.x - MENU_WIDTH;
    ...
    if (spaceBottom >= menuHeight + VIEWPORT_PADDING) {
      ...
      // Place where there's more space
