...
import { ImageIcon, FileText, Download, RefreshCw, AlertCircle } from 'lucide-react';
...
// Utility function to get file icon emoji based on MIME type
...
// Component to display a single image item
function ImageItem({ image, onClick }: { image: Attachment, onClick: () => void }) {
  return (
      <button
          key={image.id}
...
// Component to render the image grid
function ImageGrid({
                     images,
                     maxVisible,
                     onViewAll,
...
// Component to display a single file item
function FileItem({ file }: { file: Attachment }) {
  return (
      ...
      >
        <div className="flex h-10 w-10 items-center justify-center rounded-lg bg-card border border-border text-lg flex-shrink-0">
...
// Component to render the files list
function FilesList({
                     files,
                     maxVisible,
                     onViewAll,
...
function SharedAttachmentsError({ onRetry }: { onRetry?: () => void }) {
  return (
  ...
  if (isError) {
    return (
        <div className="space-y-3">
          ...
          <SharedAttachmentsError onRetry={onRetry} />
        </div>
    );
  }
